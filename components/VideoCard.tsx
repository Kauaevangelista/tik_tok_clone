...
const VideoCard: NextPage<IProps> = ({ post, isShowingOnHome }) => {
  ...
  return (
    <div className='flex flex-col border-b-2 border-gray-200 dark:border-bgDark pb-6'>
      <div>
          ...
          <div>
            <Link href={`/profile/${post.postedBy._id}`}>
              <div className='flex items-center gap-2'>
                <p className='flex gap-2 items-center md:text-md font-bold text-primary dark:text-white'>{post.postedBy.userName} {` `}
      ...
      <div className='lg:ml-20 flex gap-4 relative'>
        <div className='rounded-3xl' onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
          <Link href={`/detail/${post._id}`}>
            <video src={post.video.asset.url} ref={videoRef} loop className='lg:w-[600px] h-[300px] md:h-[400px] lg:h-[528px] w-[200px] rounded-2xl cursor-pointer bg-gray-100 dark:bg-[#282828]'></video>
          </Link>
